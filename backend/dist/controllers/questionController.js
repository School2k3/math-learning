...
function buildQuestionTtsPrompt(question) {
...
const questionController = {
    ...
    getQuestionsByLesson: async (req, res) => {
        try {
    ...
    getPracticeQuestionsByLesson: async (req, res) => {
        try {
            const { lessonId } = req.params;
            const questions = await prisma.question.findMany({
                where: {
    ...
    getQuestionAudio: async (req, res) => {
        try {
            const { id } = req.params;
            const question = await prisma.question.findUnique({
                where: { id: parseInt(id) },
    ...
    generateQuestionAudio: async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (!force && question.audioUrl) {
                sendSuccessResponse(res, { id: question.id, audioUrl: question.audioUrl, cached: true }, 'Question audio already exists');
                return;
            }
            if (!question.answers.length) {
                sendBadRequestResponse(res, 'Question must have answers before generating audio');
                return;
            }
    ...
    createQuestion: async (req, res) => {
        try {
            ...
            const question = await prisma.$transaction(async (tx) => {
                ...
                const createdAnswers = await Promise.all(answers.map(answer => tx.answer.create({
                    data: {
                        ...answer,
    ...
    updateQuestion: async (req, res) => {
        try {
            ...
            if (questionData.audioUrl !== undefined)
                updateData.audioUrl = questionData.audioUrl;
            if (questionData.explanationText !== undefined)
                updateData.explanationText = questionData.explanationText;
            if (questionData.explanationImg !== undefined)
                updateData.explanationImg = questionData.explanationImg;
            if (questionData.grade !== undefined)
                updateData.grade = questionData.grade;
            if (questionData.type !== undefined)
                updateData.type = questionData.type;
            if (questionData.answerType !== undefined)
                updateData.answerType = questionData.answerType;
            if (questionData.lessonId !== undefined)
                updateData.lessonId = questionData.lessonId;
            // Update the question
            const updatedQuestion = await prisma.question.update({
                where: { id: questionId },
                data: updateData,
    ...
    updateQuestionAnswers: async (req, res) => {
        try {
            const { id } = req.params;
            const questionId = parseInt(id);
