...
interface QuestionAudioInput {
  questionId?: number;
  questionText: string;
  answers: { answerText: string }[];
}

export interface SynthesizedQuestionAudioResult {
  audioUrl: string;
  mimeType: string;
}

function buildPromptFromQuestion(input: QuestionAudioInput): string {
  const lines: string[] = [
    'Hãy đọc rõ ràng như giáo viên tiểu học.',
...
async function generateAudioBuffer(prompt: string): Promise<{
  buffer: Buffer;
  mimeType: string;
...
function decodeInlineAudio(inlineData: { data?: string; mimeType?: string }) {
  if (!inlineData.data) {
...
function convertPcmToWav(base64: string, mimeType: string): Buffer {
...
function parsePcmMimeType(mimeType: string): WavOptions {
  ...
  for (const param of params) {
    ...
    if (key === 'rate') {
      ...
      if (!Number.isNaN(parsed)) options.sampleRate = parsed;
    }
    if (key === 'channels') {
      const parsed = parseInt(value, 10);
      if (!Number.isNaN(parsed)) options.numChannels = parsed;
    }
    if (key === 'bitrate' || key === 'bits') {
      const parsed = parseInt(value, 10);
      if (!Number.isNaN(parsed)) options.bitsPerSample = parsed;
    }
  }
...
function createWavHeader(dataLength: number, options: WavOptions) {
...
function uploadAudioToCloudinary(
  buffer: Buffer,
  options: { questionId?: number; fileExtension: string }
): Promise<{ url: string }> {
...
export async function synthesizeQuestionAudio(
  input: QuestionAudioInput
): Promise<SynthesizedQuestionAudioResult> {
